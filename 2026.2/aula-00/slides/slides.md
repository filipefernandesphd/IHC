---
theme: slidev-theme-tahta
addons:
  - slidev-addon-citations
title: Programa Analítico
aspectRatio: 4/3
info: |
  Visão geral da disciplina Interação Humano-Computador - 2026/2.
themeConfig:
  variant: minimal
mdc: true
routerMode: hash
preloadImages: false
biblio:
  filename: references.bib
  show_full_bib: true
  show_id: false
layout: academic-cover
---

---
layout: section
index: "01"
title: Sobre Mim
---

---
layout: image
side: right
image: ./vertical_profile.png
title: Filipe Fernandes
---
**Pessoal:**
- Itaperuna/RJ
- Baterista que toca outros instrumentos
- Design Inteligente
- CEO de startup futura

**Profissional:**
- IF Sudeste MG - Campus Manhuaçu (out/2016 - Atual)
- Mestrado e Doutorado - COPPE/UFRJ (2014 - 2023)
- Pós-doutorado - UFMG (2025 - 2026)
- Especialista em Computação Quântica - SENAI/CIMATEC (2025 - 2026)

---
layout: image
side: right
image: ./quantum_metaverse.png
title: Atuação
---
- **Pesquisador**:
  - Quantum Software Engineering (QSE)
  - Metaverse Engineering
  - AI4SE e SE4AI

- **Coodenador de projetos de pesquisa:**
  - IF Sudeste MG
  - FAPEMIG

- Líder do **LIPES**

- **Steering Committee**: [I Workshop Brasileiro de Engenharia de Software Quântico (WQSE)](https://wqseworkshop.github.io/WQSE2026/organization.html)

- **Editor Assistente**: SBC Reviews

- **Comitê de programa**: conferências nacionais e internacionais

- **Revisor**: revistas científicas nacionais e internacionais



---
layout: section
index: "02"
title: Informações Gerais
---

---
layout: default
title: Sobre o Programa Analítico

---

- Art. 9º §2º \[...\] deverá ser feito pelos professores \[...\] levando-se em consideração o PPC, devendo conter [@RAG2018]:

|                                                                                                                                  |                                                                |
| -------------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------- |
| Curso, semestre letivo, disciplina, código, carga horária e pré-requisitos                                                       | Número de horas e aulas previstas por semestre                 |
| Período de execução e nome do(s) professor(es)                                                                                   | Metodologia                                                    |
| Ementa                                                                                                                           | Recursos didáticos                                             |
| Objetivos                                                                                                                        | Avaliação                                                      |
| Conteúdo programático discriminando a quantidade de aulas por conteúdo, separando as aulas teóricas e práticas, quando aplicável | Bibliografia básica (mínimo de 3) e complementar (mínimo de 5) |

---
title: Sobre o Programa Analítico
---
- Art. 9º §1º \[...\] deverá ser apresentado aos discentes **na primeira aula da disciplina** [@RAG2018]
- Art. 9 §3º \[...\] **poderão ser modificados** a partir de demandas identificadas no decorrer da disciplina [@RAG2018]

---
title: Informações do Curso
---

- Curso: Bacharelado em Sistemas de Informação
- Modalidade de oferta: Presencial
- Área de conhecimento: Computação
- Turno de oferta: Integral
- Número de períodos: 8

---
title: Regras para Aprovação ou Reprovação
---

Art. 37 [@RAG2018]

- **APROVADO**: nota da disciplina >= 6,0 **E** frequência >= 75%
- **REPROVADO**: nota da disciplina < 4,0 **OU** frequência < 75%
- **EXAME FINAL**: opcional ao discente com nota >= 4,0 **E** nota < 6,0 **E** frequência >= 75%

---
title: Regras para Frequência
---
- Art. 36. A frequência às aulas e as demais atividades acadêmicas serão obrigatórias [@RAG2018]
- §1º Casos aceitos para **abono de faltas**:
  - Alunos reservistas
  - Oficial ou Aspirante-a-Oficial da Reserva, convocado para o serviço ativo
  - Aluno com representação que tenha participado de reuniões da Comissão Nacional de Avaliação da Educação Superior – CONAES
- O registro da frequência é por aula (45min) e **não** por dia

---
layout: section
index: "03"
title: Informações da Disciplina

---

---
title: Informações da Disciplina
---

- Período: 4º semestre
- Natureza: obrigatória
- Código: INF03079
- Carga-horária: 60 horas
- Número de aulas: 80 aulas
- Pré-requisito: não se aplica

---
title: Por que estudar Interação Humano-Computador?
kicker: Motivação
---
- Entender como as pessoas interagem com sistemas computacionais
- Projetar interfaces intuitivas, acessíveis e centradas no usuário
- Avaliar e melhorar continuamente a experiência do usuário (UX)
- Criar software que as pessoas realmente queiram utilizar

---
layout: quote
quote: As pessoas não interagem com algoritmos. Elas interagem com experiências
---

---
title: Objetivo
---

Projetar experiências de interação que atendam às necessidades dos usuários, utilizando métodos, técnicas e tecnologias de IHC

---
title: Metodologia
---
- Aulas Expositivas (TDH)
  - **T**eoria
  - **D**esenvolvimento
  - **H**ands-on
- Sala de aula invertida (*flipped classroom*)
- Aprendizagem baseada em projetos (trabalho final)

---
title: Recursos Didáticos
---

- **Avisos, frequência e envio de atividades**: SIGAA
- **Materiais**: [https://filipefernandesphd.com/teaching](https://filipefernandesphd.com/teaching)
- **Recursos em sala de aula**: computador, datashow, lousa e pincéis, tablet, slides, figma, dentre outros recursos digitais

---
title: Bibliografias
---
| Bibliografia Básica                                                                                                                                                                                                                                                                                                                | Bibliografia Complementar                                                                                                                                                                                                                                                                             |
| ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| BARRETO, Jeanine dos S.; JR., Paulo A. P.; BARBOZA, Fabrício F. M.; et al. **Interface humano-computador**. Porto Alegre: SAGAH, 2018. E-book. ISBN 9788595027374. | TEIXEIRA, Fabrício. **Introdução e boas práticas em UX Design**. São Paulo: Casa do Código, 2017. 263 p. ISBN 9788566250480.                                                                                                                                                                          |
| ROGERS, Yvonne; SHARP, Helen; PREECE, Jennifer. **Design de interação: além da interação humano-computador**. 3. ed. Porto Alegre: Bookman, 2013. 585 p. ISBN 978-85-8260-006-1.                                                                                                                                                   | ALVES, William P. **Unity: design e desenvolvimento de jogos**. Rio de Janeiro: Editora Alta Books, 2019. E-book. ISBN 9786555200102. |
| LOWDERMILK, Travis. **Design centrado no usuário: um guia para desenvolvimento de aplicativos amigáveis**. São Paulo: Novatec, 2013. 182 p. ISBN 978-85-7522-366-6.                                                                                                                                                                | FIALHO, A. B. **Realidade Virtual e Aumentada: Tecnologias para Aplicações Profissionais**. São Paulo: Saraiva, 2018. 144 p. ISBN 978853652596-9.                                                                                                                                                     |
|                                                                                                                                                                                                                                                                                                                                    | SOBRAL, Wilma S. **Design de Interfaces: Introdução**. São Paulo: Saraiva, 2019. E-book. ISBN 9788536532073.                          |
|                                                                                                                                                                                                                                                                                                                                    | OLIVEIRA, Alexandre Dutra de; SILVA, Luis Fernando da. **Desenvolvimento de Software: Aplicativo Comercial com C# e Camadas – Cadastros Básicos**. São Paulo: Edição do Autor, 2017. 111 p. ISBN 9788591501472.                                                                                       |


---
title: Ementa
---
- Princípios de Interação Homem-computador
- Aspectos cognitivos
- Interação social e emocional
- Interfaces
- Processo de análise e design da interação
- Design centrado no usuário
- Avaliação da interação
- Realidade virtual, aumentada e misturada
- Desenvolvimento de aplicações por meio de interfaces multimodais

---
title: Cronograma
---

Disponível em [https://filipefernandesphd.github.io/IHC/2026.2/](https://filipefernandesphd.github.io/IHC/2026.2/)

---
title: Atendimento
---
- Quintas-feiras de 13h às 17h
- Agendamento: [https://forms.gle/2VPe4UfohKUxa7Du9](https://forms.gle/2VPe4UfohKUxa7Du9)

---
title: Avaliações
---
- **Trabalho 1**: 3pts
- **Trabalho 2**: 2pts
- **Projeto Final**: 5pts

<font size=4pt><i>Datas disponíveis em <a href="https://filipefernandesphd.github.io/IHC/2026.2/" target="_blank">https://filipefernandesphd.github.io/IHC/2026.2/</a></i></font>

---
layout: section
index: "04"
title: Trabalho 1
---

---
layout: default
title: Objetivo
kicker: Trabalho 1
---
- Análise crítica de sistema com base nos conceitos de IHC

---
title: Formatos
kicker: Trabalho 1
---
- Relatório Técnico: 2,5pts
- Apresentação: 0,5pt

---
title: Relatório Técnico
kicker: Trabalho 1
---
- **Trabalho individual**
- **Template**: [SBC em LaTeX](https://www.sbc.org.br/wp-content/uploads/2024/07/modelosparapublicaodeartigos.zip) (usar Overleaf)
- **Objetivo**: analisar um sistema conforme os conceitos de IHC transmitidos
  - Não será permitida a análise do mesmo sistema por mais de um aluno
- **Evidências**: comprovar no relatório as análises realizadas (screenshots de telas, descrição em detalhes do que foi feito ...)
- **Entrega**: um arquivo **.zip** com
  - o projeto completo do Overleaf zipado
  - o pdf da versão final do relatório


---
title: Seções (1)
kicker: Trabalho 1
---
- **Objetivo**: definir qual sistema será analisado
- **Interação**: apresentar qual é o tipo de interação: sistema, parceiro de discurso, mídia e ferramenta
- **Interface**: apresentar como é o contato físico durante a interação
- **Affordance**: listar 5 exempos de *affordances* da interface
- **Usabilidade**: aplicar as Heurísticas de Nielsen
- **User Experience (UX)**: aplicar o *User Experience Questionnaire (UEQ)*
- **Acessibilidade**: aplicar Accessible Usability Scale (AUS)

---
title: Seções (2)
kicker: Trabalho 1
---
- **Cognição**: analisar 3 cenários para cada tipo de sistema repondendo as perguntas abaixo

| **Sistema 1 (Pensamento Intuitivo)** | **Sistema 2 (Pensamento Reflexivo)** |
|--------------------------------------|--------------------------------------|
| **Qual cenário/funcionalidade foi analisada?** | **Qual cenário/funcionalidade foi analisada?** |
| **Por que essa interação é predominantemente intuitiva?** | **Por que essa tarefa exige raciocínio deliberado?** |
| **Quais elementos da interface favorecem esse comportamento?**<br>(ícones, cores, localização dos componentes, padrões conhecidos, hábitos, metáforas, feedback etc.) | **O esforço cognitivo é justificável?** |
| **Essa escolha de design é adequada? Justifique.** | **A interface oferece apoio suficiente para que o usuário tome uma decisão correta?** |

---
title: Seções (3)
kicker: Trabalho 1
---
- **Engenharia Semiótica**:
  - Descobrir qual é a metamensagem que o designer parace transmitir
  - Apresentar para qual tipo de usuário (características) o sistema foi projetado
  - Demonstrar se os objetivos do sistema são comunidados claramente
  - Identificar 5 signos e para cada um responder:
    - O que esse signo representa?
    - Seu significado é claro?
    - Pode gerar interpretações diferentes?

---
title: Seções (4)
kicker: Trabalho 1
---
- **Engenharia Semiótica (continuação)**:
  - Demonstrar se o sistema comunica os problemas de forma clara
  - Apresentar se sistema oferece recursos de ajuda ao usuário
  - Identificar 3 evidências da intenção do designer
    - Ex: No e-commerce, o botão “Comprar Agora” aparece destacado em verde para incentivar compras rápidas
  - Identificar 3 situações em que a comunicação falha
    - Ex: Não entendi o que este botão faz; Não sei o que aconceteu quando cliquei no botão validadar

---
title: Seções (5)
kicker: Trabalho 1
---
- **Design baseado em Objetivos**: listar 5 objetivos do sitema
  - Ex: pagar uma conta antes do vencimento
- **Design baseado em Cenários**: para cada objetivo, liste 3 cenários
  - Ex: Objetivo: pagar uma conta antes do vencimento
    - Cenários: usuário pode estar em casa; no trabalho ou no ônibus

---
title: Apresentação
kicker: Trabalho 1
---
- Apresentação de até 5 min


---
layout: section
index: "05"
title: Trabalho 2
---

---
title:
kicker: Trabalho 2
---
- **Objetivo**: desenvolvimento de protótipo com A-Frame
- **Pontuação**: 2pts

---
layout: section
index: "06"
title: Projeto Final
---

---
layout: default
title: Objetivos
---
- Dominar os conceitos de Interação Humano-Computador
- Aprender a utilizar IAs de forma crítica e responsável
- Desenvolver protótipos para interfaces inovadoras e multimodais
- Aprendizam baseada em projetos
- Aplicar uma metodologia experimental simples para analisar evidências

---
title: Requisitos
---
- **Trabalho individual**
- **Prototipação funcional** (MVP) de **sistema multimodal com IA**, sendo **imersivo ou IoT**
  - *Implementação do sistema com dados simulados e sem persistência*
  - *Foco na interface e interação do usuário*
- **Integração de pelo menos 3 modalidades**: voz, gestos, toque, visão computacional, senores, olhar, feedback háptico etc.
- **Equipamentos disponíveis**: 2 Meta Quests; 1 scanner 3D; 1 câmera 360º
  - O uso dos equipamentos é permitido **somente** no horário da aula e na presença do professor

---
title: Exemplos de protótipos (1)
---
- **Multimodal Imersivo com IA**
  - **Tutor Imersivo de Laboratório Virtual**: atua como tutor inteligente, responde perguntas, identifica erros durante a execução de experimentos e adapta o nível de dificuldade
  - **Museu Inteligente em Realidade Virtual**: reconhece o interesse do visitante, explica obras, responde perguntas e personaliza o roteiro da visita

---
title: Exemplos de protótipos (2)
---
- **IoT Multimodal com IA**
  - **Guarda-Roupa Inteligente**: um sistema que recomende roupas com base no clima, agenda e preferências do usuário utilizando câmera, RFID e IA
  - **Assistente Inteligente de Cozinha**: um dispositivo semelhante a uma Alexa, instalado na bancada da cozinha, que auxilia durante o preparo de alimentos

---
title: Formatos
---
- Desenvolvimento do projeto em aula
- Relatório técnico
- Apresentação

---
title: Desenvolvimento do projeto em aula
---
- Desenvolvimento em aula (conforme [programa analítico](https://filipefernandesphd.github.io/IHC/2026.2)) e extraclasse
- A data de início não se restringe ao definido no programa analítico

---
title: Relatório técnico (1)
---
- **Template**: [SBC em LaTeX](https://www.sbc.org.br/wp-content/uploads/2024/07/modelosparapublicaodeartigos.zip) (usar Overleaf)
- **Acesso aos artefatos**: disponibilizar todos os artefatos utilizados no repositório do GitHub e citá-lo (com link) no relatório técnico
- **Limite**: 10 a 12 páginas, além das referências
- **Entrega**: um arquivo **.zip** com
  - o projeto completo do Overleaf zipado
  - o projeto da implementação do GitHub zipado
  - o pdf da versão final do relatório

---
title: Relatório técnico (2)
---
- **Seções**:
  - **Resumo**: síntese do objetivo do trabalho e resultados da avaliação
  - **Objetivo**: definir qual sistema será desenvolvido
  - **Desenvolvimento do Protótipo**: detalhamento do desenvolvimento do sistema (desde a concepção, lista de funcionalidades, processo de desenvolvimento, tecnologias etc.)
  - **Avaliação do Protótipo**: aplicar algum método de avaliação de IHC (avaliação da usabilidade, UX, comunicabilidade etc.)
  - **Resultados e Discussão**: apresentar os resultados da avaliação e discuti-los
  - **Conclusão**: visão geral do desenvolvimento e principais achados da avaliação
  - **Referências**: trabalhos citados no texto

---
title: Declaração de Uso de IA
---
- IA pode ser utilizada para gerar dados da avaliação
  - Todos os artefatos (prompts, arquivos etc.) e detalhes devem estar no repostiório do GitHub
- O uso de IA para qualquer fim deve ser declarado na seção de *Agradecimentos*, sem numeração e antes das referências

---
title: Apresentação
---
- Apresentação de até 5 min

---
layout: section
index: "07"
title: Acordos Pedagógicos

---

---
title: Acordos Pedagógicos (1)
---
- **Respeito acima de tudo** [@regulamento_conduta_discente]:
  - Ao professor
  - Aos colegas da turma
  - Todos em geral

---
title: Acordos Pedagógicos (2)
---
- **Registro da frequência**:
  - **Obrigatoriamente** através da lista de presença
  - Bom senso na permanência em sala de aula

- **Aprovação**:
  - Conforme normas institucionais [@RAG2018]

---
title: Acordos Pedagógicos (3)
---
- **Reposições do curso (oferta integral)** [@PPCBSI2025]:
  - Período vespertino, conforme definido no [cronograma](https://filipefernandesphd.github.io/IHC/2026.2/)

---
title: Acordos Pedagógicos (4)
---
- **Direitos e deveres do discente** [@regulamento_conduta_discente]:
  - Art. 15 inciso I: "*Conhecer e cumprir as diretrizes no Regulamento de Conduta Discente do IF Sudeste MG e demais regulamentos e normas institucionais*"
  - Art. 19: "*O ato de indisciplina se caracterizará pelo não cumprimento de um ou mais incisos constantes no *art. 15* ou a prática de um ou mais incisos constantes no art. 16*"

**Acesse o [Chatbot não oficial](https://filipefernandesphd.com/teaching) para obter mais informações**

---
title: Acordos Pedagógicos (5)
---

- **Materiais**:
  - Uso **obrigatório** dos materiais informados pelo professor

---
layout: section
index: "08"
title: Gravação das aulas
---

---
title: Gravação das aulas (1)
---
- Iniciativa **exclusivamente** do Prof. Filipe Fernandes

- Registro **exclusivamente** do Prof. Filipe Fernandes:
  - Tela do computador
  - Imagem e voz

- **A imagem, voz ou identificação dos estudantes não serão gravadas intencionalmente**

---
title: Gravação das aulas (2)
---

- **Objetivos**:
  - Produção de conteúdo educacional de autoria do Prof. Filipe Fernandes
  - Disponibilização de **material de apoio** para os estudantes da disciplina
    - Trasmissão ao vivo no [YouTube e outras plataformas](https://filipefernandesphd.com/)

---
title: Gravação das aulas (3)
---
- **IMPORTANTE!**
  * As transmissões ao vivo e as gravações são uma **iniciativa exclusiva do professor**, destinada ao apoio aos estudos e à produção de conteúdo educacional
  * **Não** configuram modalidade institucional de ensino
  * **Não** substituem as aulas presenciais
  * É **vedada** a utilização das transmissões ao vivo ou das gravações como justificativa para faltas às aulas presenciais

---
layout: default
title: Referências

---

<BiblioList />

---
layout: feature
kicker: Encerramento
title: Obrigado!
columns: 2
features:

- { icon: "lucide:globe", desc: filipefernandesphd.com }
- { icon: "lucide:instagram", desc: "@filipfernandesphd" }

---
